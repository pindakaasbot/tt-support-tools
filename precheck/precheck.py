...
if sram_drw.count > 0
  ...
  gatpoly_drw     = gatpoly_drw.not(sram_excl)
  psd_drw         = psd_drw.not(sram_excl)
  nsd_drw         = nsd_drw.not(sram_excl)
  nsd_block       = nsd_block.not(sram_excl)
  cont_drw        = cont_drw.not(sram_excl)
  nwell_drw       = nwell_drw.not(sram_excl)
  nbulay_drw      = nbulay_drw.not(sram_excl)
  pwell_block     = pwell_block.not(sram_excl)
  salblock_drw    = salblock_drw.not(sram_excl)
  thickgateox_drw = thickgateox_drw.not(sram_excl)

  # BEOL drawing layers
  metal1_drw      = metal1_drw.not(sram_excl)
  metal2_drw      = metal2_drw.not(sram_excl)
  metal3_drw      = metal3_drw.not(sram_excl)
  metal4_drw      = metal4_drw.not(sram_excl)
  metal5_drw      = metal5_drw.not(sram_excl)
  via1_drw        = via1_drw.not(sram_excl)
  via2_drw        = via2_drw.not(sram_excl)
  via3_drw        = via3_drw.not(sram_excl)
  via4_drw        = via4_drw.not(sram_excl)

  # Pin layers (prevent orphaned pin-without-drawing violations)
  activ_pin       = activ_pin.not(sram_excl)
  gatpoly_pin     = gatpoly_pin.not(sram_excl)
  metal1_pin      = metal1_pin.not(sram_excl)
  metal2_pin      = metal2_pin.not(sram_excl)
  metal3_pin      = metal3_pin.not(sram_excl)
  metal4_pin      = metal4_pin.not(sram_excl)
  metal5_pin      = metal5_pin.not(sram_excl)
end
